...
$listItems = Get-PnPListItem -List $list -PageSize 1000 -Fields "File","OrganisationIntranets","Title","_UIVersionString"| Where-Object {$_["OrganisationIntranets"].Count -gt 0}

foreach($item in $listItems)
{  
    $currentVersionNumber = $item["_UIVersionString"]

    Set-PnpTaxonomyFieldValue -ListItem $item -InternalFieldName "OrganisationIntranetsContentEditorInput" -Terms @{}
